...
public class ByName extends BrowserSelection
{
	...
	{  
	  try  
	  {  
		  Dropdown_Select_Using_Name(finalxpath,screenshotname,Integer.parseInt(ddl_value));
	  }  
	  catch(NumberFormatException nfe)  
	  {
		 Dropdown_Select_Using_Name(finalxpath,screenshotname,(ddl_value));  
	    return false;  
	  }  
	  return true;  
	}
	...
	public static void Dropdown_Select_Using_Name(String finalxpath, String screenshotname, int index_value) throws IOException, InterruptedException 
	{
		try
		{
		dynamicwait.WaitUntilElement(By.name(finalxpath));
		WebElement clickfn = driver.findElement(By.name(finalxpath));
		Jexecutor.highlightElement(clickfn);
		Select select = new Select(driver.findElement(By.name(finalxpath)));
